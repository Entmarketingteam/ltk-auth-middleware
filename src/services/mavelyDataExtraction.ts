...
import { getPlatformTokens } from './platformStorage.js';
 ...
 */
export async function extractMavelyData(
  userId: string,
  startDate: string,
  endDate: string
): Promise<MavelyDataExtractionResult> {
  ...
  try {
    ...
    // Get stored tokens
    const tokens = await getPlatformTokens(userId, 'MAVELY');
    if (!tokens) {
      return {
        success: false,
 ...
 */
async function extractAnalyticsFromPage(page: Page): Promise<MavelyAnalyticsData[]> {
  try {
    // Extract data from the page
    const data = await page.evaluate(() => {
      const results: MavelyAnalyticsData[] = [];
      
      // Try to find quick stats
      const statsElements = document.querySelectorAll('[class*="stat" i], [class*="metric" i], [class*="card" i]');
      
      const today = new Date().toISOString().split('T')[0];
      const dataPoint: MavelyAnalyticsData = {
        date: today,
      };
      ...
      tables.forEach(table => {
        ...
        rows.forEach(row => {
          ...
          if (cells.length >= 2) {
            const rowData: MavelyAnalyticsData = {
 ...
 */
export async function exportMavelyCSV(
  userId: string,
  startDate: string,
  endDate: string
